...
const makeChatsSocket = (config) => {
    ...
    const fetchStatus = async (...jids) => {
        ...
        const users = await Promise.all(jids.map(async (jid) => {
            const user = new WAUSync_1.USyncUser().withId(jid);
            const tcTokenContent = (0, WABinary_1.isJidUser)(jid) || (0, WABinary_1.isLidUser)(jid)
                ? await (0, tc_token_utils_1.buildTcTokenFromJid)({
                    keys: authState.keys,
                    jid,
                    resolveLid: pn => cache_utils_1.default.lidCache.get((0, WABinary_1.jidNormalizedUser)(pn))
                })
                : undefined;
            return (tcTokenContent === null || tcTokenContent === void 0 ? void 0 : tcTokenContent.length) ? user.withContent(tcTokenContent) : user;
        }));
    ...
    const newAppStateChunkHandler = (isInitialSync) => {
        return {
            async onMutation(mutation) {
                var _a;
                if (((_a = mutation.index) === null || _a === void 0 ? void 0 : _a[0]) === cs_token_utils_1.NCT_SALT_SYNC_INDEX) {
                    await (0, cs_token_utils_1.handleNctSaltMutation)({ mutation, keys: authState.keys, logger });
                    return;
                }
    ...
    const persistNctSaltMutations = async (mutationMap) => {
        var _a;
    ...
    const profilePictureUrl = async (jid, type = 'preview', timeoutMs) => {
        ...
        const isSelf = jid === (0, WABinary_1.jidNormalizedUser)(me === null || me === void 0 ? void 0 : me.id) || jid === (0, WABinary_1.jidNormalizedUser)(me === null || me === void 0 ? void 0 : me.lid);
        const tcTokenContent = !isSelf && ((0, WABinary_1.isJidUser)(jid) || (0, WABinary_1.isLidUser)(jid))
            ? await (0, tc_token_utils_1.buildTcTokenFromJid)({
                keys: authState.keys,
                jid,
                resolveLid: pn => cache_utils_1.default.lidCache.get((0, WABinary_1.jidNormalizedUser)(pn))
     ...
     */
    const presenceSubscribe = async (toJid, tcToken) => {
        let content;
        if (tcToken) {
    ...
    const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
        ...
        async function doAppStateSync() {
            if (!authState.creds.accountSyncCounter) {
                ...
                const result = await resyncAppState(Types_1.ALL_WA_PATCH_NAMES, true);
                if (result === null || result === void 0 ? void 0 : result.failedCollections.length) {
                    logger.warn({ collections: result.failedCollections }, 'app state sync inicial incompleto; contador não será avançado');
                    return;
                }
