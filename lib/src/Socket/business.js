...
const makeBusinessSocket = (config) => {
    ...
    const sendStatus = async (content, options) => {
        const isSupportedContent = typeof content === 'object' && content !== null && ('text' in content || 'image' in content || 'video' in content);
        if (!isSupportedContent) {
            throw new TypeError('sendStatus aceita apenas conteúdo de texto, imagem ou vídeo');
        }
        const statusJidList = [
            ...new Set(((options === null || options === void 0 ? void 0 : options.statusJidList) || [])
                .map(WABinary_1.jidNormalizedUser)
                .filter(jid => (0, WABinary_1.isJidUser)(jid) || (0, WABinary_1.isLidUser)(jid)))
        ];
        ...
        return sock.sendMessage(WABinary_1.STORIES_JID, content, {
            ...messageOptions,
            broadcast: true,
            statusJidList
        });
